...
class Environment(gym.Env):
    ...
    def reset(self):
        ...
        self.ur5 = pybullet_utils.load_urdf(
            p, os.path.join(self.assets_root, UR5_URDF_PATH), [0, 0.25, 0],)

        self.ur5_1 = pybullet_utils.load_urdf(
            p, os.path.join(self.assets_root, UR6_URDF_PATH),  [1, -0.25, 0],  baseOrientation=[0.,0.,1.,0.,])

        self.ee = self.task.ee(self.assets_root, self.ur5_1, 9, self.obj_ids)
        ...
        n2_joints = p.getNumJoints(self.ur5_1)
        joints_2 = [p.getJointInfo(self.ur5_1, i) for i in range(n2_joints)]
        self.joints2 = [j[0] for j in joints_2 if j[2] == p.JOINT_REVOLUTE]

        # Move robot to home joint configuration.
        for i in range(len(self.joints)):
            p.resetJointState(self.ur5, self.joints[i], self.homej[i])
        for i in range(len(self.joints2)):
            p.resetJointState(self.ur5_1, self.joints2[i], self.homej[i])
        # Reset end effector.
        self.ee.release()
    ...
    def movej(self, targj, speed=0.01, timeout=5):
        ...
        while (time.time() - t0) < timeout:
            currj = [p.getJointState(self.ur5_1, i)[0] for i in self.joints2]
            ...
            stepj = currj + v * speed
            gains = np.ones(len(self.joints2))
            p.setJointMotorControlArray(
                bodyIndex=self.ur5_1,
                jointIndices=self.joints2,
