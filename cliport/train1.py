...
from cliport.dataset1 import RavensDataset, RavensMultiTaskDataset
import hydra
...
@hydra.main(config_path="./cfg", config_name='train1')
def main(cfg):
    # Logger
    ...
    checkpoint_path = os.path.join(cfg['train']['train_dir'], 'checkpoints-cliport_net')
    ...
    # print(max_epochs, last_checkpoint)
    trainer = Trainer(
        gpus=cfg['train']['gpu'],
    ...
    if 'multi' in dataset_type:
        ...
        # 不同模型的训练地址需要改
        train_ds = RavensDataset(os.path.join(data_dir, '{}-2complete-train'.format(task)), cfg, n_demos=n_demos, augment=True)
        val_ds = RavensDataset(os.path.join(data_dir, '{}-2complete-val'.format(task)), cfg, n_demos=n_val, augment=False)
