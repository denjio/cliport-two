...
class RavensDataset(Dataset):
    ...
    def __init__(self, path, cfg, n_demos=0, augment=False):
        ...
        color_path = os.path.join(self._path, 'action1')
    ...
    def add(self, seed, episode):
        ...
        """
        color, depth, action1, action2, reward, info = [], [], [], [], [], []
        for obs, act1, act2, r, i in episode:
        ...
        dump(color, 'color')
        dump(depth, 'depth')
        dump(action1, 'action1')
        dump(action2, 'action2')
        dump(reward, 'reward')
        dump(info, 'info')
    ...
    def load(self, episode_id, images=True, cache=False):
        ...
        seed = None
        path = os.path.join(self._path, 'action1')
        for fname in sorted(os.listdir(path)):
            ...
            if f'{episode_id:06d}' in fname:
                ...
                action1 = load_field(episode_id, 'action1', fname)
                action2 = load_field(episode_id, 'action2', fname)
                ...
                for i in range(len(action1)):
                    ...
                    episode.append((obs, action1[i], action2[i], reward[i], info[i]))
    ...
    def get_image(self, obs, cam_config=None):
        ...
        # print('camp', cmap.shape, 'hamp', hmap.shape) # camp (320, 160, 3) hamp (320, 160)
    ...
    def process_sample(self, datum, augment=True):
        ...
        if act1:
            p0_xyz, p0_xyzw = act1['pose0']
            p1_xyz, p1_xyzw = act1['pose1']
        ...
        p00, p11 = None, None
        p00_theta, p11_theta = None, None
        perturb_params1 = None

        if act2:
            p00_xyz, p00_xyzw = act2['pose0']
            p11_xyz, p11_xyzw = act2['pose1']
            p00 = utils.xyz_to_pix(p00_xyz, self.bounds, self.pix_size)
            p00_theta = -np.float32(utils.quatXYZW_to_eulerXYZ(p00_xyzw)[2])
            p11 = utils.xyz_to_pix(p11_xyz, self.bounds, self.pix_size)
            p11_theta = -np.float32(utils.quatXYZW_to_eulerXYZ(p11_xyzw)[2])
            p11_theta = p11_theta - p00_theta
            p00_theta = 0
            # Data augmentation.
            if augment:
                img, _, (p00, p11), perturb_params1 = utils.perturb(img, [p00, p11], theta_sigma=self.aug_theta_sigma)
        sample = {
            'img': img,
            'p0': p0, 'p0_theta': p0_theta,
            'p1': p1, 'p1_theta': p1_theta,
            'perturb_params': perturb_params,
            'p00': p00, 'p00_theta': p00_theta,
            'p11': p11, 'p11_theta': p11_theta,
            'perturb_params1': perturb_params1
        }
    ...
    def process_goal(self, goal, perturb_params,perturb_params1):
        # Get goal sample.
        (obs, act1, act2, _, info) = goal
        img = self.get_image(obs)
    ...
    def __getitem__(self, idx):
        ...
        if len(episode)-1 != 0:
            ...
            i = 0
        # 这里随机选择动作序列的其中一个动作，通过深度学习方法去学习放的位置，而不人为选择恰当的位置，这里长度不-1会导致取到action=None的地方

        g = i+1 if is_sequential_task else -1
        sample, goal = episode[i], episode[g]

        # Process sample. 这里goal在训练的时候没有用过，需要注意
        sample = self.process_sample(sample, augment=self.augment)
        goal = self.process_goal(goal, perturb_params=sample['perturb_params'], perturb_params1=sample['perturb_params1'])
